...
# problematic files
problematic = ["airland6.txt","airland9.txt","airland10.txt","airland11.txt","airland12.txt","airland13.txt"]
...
def ALP_dynamic(sequence, Solution_List, F0, F1, F2, E, T, L, g, h, S):
    ...
    m.addConstrs((delta[i, j] + delta[j, i] == 1 for i in F1 if i != -1 for j in F1 if i != -1 if j > i), 
                 name="Sequence")
    # constraint for keeping the given sequence
    for i in range(P):
        for j in range(P):
            if (i in sequence and j in sequence):
                # if i lands before j
...
sequence = zerolistmaker(P)

# solve ALP
R = ALP_dynamic(sequence, previous_solution, F0, F1, F2,
...
for m in range(P-1):
    ...
    #sequence
    sol = previous_solution.copy()
    '''
    Since the first planes in instance 6 and 7 are deemed to land at 0 our list would be impossible to 
    implement, as previous_solution gives 0 as landing times for planes that still have not appeared. 
    As this is the only case we are hardcoding here a workaround to make it feasible, namely changing 0 
    to 1. It would be possible however to make this workaround scalable by obtaining the index of the 0 
    landing plane from the latest landin times list.
    '''
    if sol[0] == 0:      
        sol[0] = 1
    
    for i in range(len(sol)):
        # newly appeared plane
        if i == index:
            # for the problematic instances
            if instance in problematic:   
                sol[i] = max(sol)               
            # insert the newly appeared time according to its target time
            else:                           
                sol[i] = T[i]
        if sol[i] == 0.0:
            sol[i] = 99999 
    ...
    for i in range(len(sequence)):
        sequence[i] = sol.index(seq[i])
    iteration = P - len(F0)
    sequence_cut = sequence[:iteration]

    # solve ALP
    R = ALP_dynamic(sequence_cut, previous_solution, F0, F1, F2,
                                   list(E[i] for i in range(P)), 
                                   list(T[i] for i in range(P)), 
                                   list(L[i] for i in range(P)),
                                   list(g[i] for i in range(P)), 
